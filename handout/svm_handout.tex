...
Die Idee einer Kernel-Funktion wird nun anhand eines einfachen Beispiels gezeigt.
Folgende Kernel-Funktion für $x, x' \in \mathbb{R}^2$ sei gegeben:
...
$\Phi$ transformiert in diesem Beispiel 2-dimensionale Vektoren nach $\mathbb{R}^6$.
Mittels der Kernel-Funktion kann das Skalarprodukt der transformierten Vektoren durch $K(x, x') = (1 + x^{T}x')^{2}$ berechnet werden ohne dass die Vektoren $x$ und $x'$ tatsächlich in den neuen Raum transformiert werden müssen. \\

In dem Beispiel wurde die Berechnung von 6-dimensionalen Skalarprodukten durchgeführt durch die Berechnung mittels der Kernel-Funktion, die lediglich 2-dimensionale Skalarprodukte berechnen muss.
Führt die Funktion $\Phi$ eine Transformation in eine viel höhere oder unendlich hohe Dimension aus ist der Vorteil einer solchen Kernel-Funktion sofort ersichtlich.

\subsection{Polynomieller Kernel}
Die in \autoref{sec:example_kernel} verwendete Kernel-Funktion kann verallgemeinert werden.
Seien die Eingabevektoren $x \in \mathbb{R}^d$ und die Transformationsfunktion $\Phi: \mathbb{R}^d \rightarrow \mathbb{Z}$ ein Polynom der Ordnung $Q$, dann kann eine Kernel-Funktion wie folgt definiert werden:
...
Die in \autoref{eq:poly_kernel} beschriebene Kernel-Funktion wird polynomieller Kernel genannt.
Weil durch das Polynom eine Vielzahl an Faktoren entsteht kann zur Kompensation dieser Faktoren die Kernel-Funktion mit den Skalierungsfaktoren $a$ und $b$ erweitert werden:
...
Das Ergebnis von \autoref{eq:proof_rbfk_infinite} hat bereits die für ein Skalarprodukt benötigte Symmetrie von $exp(-x^{2})$ - $exp(-x'^{2})$ und $(x)^{k}$ - $(x')^{k}$.
Die Anteile $\frac{2^k}{k!}$ können gleichmäßig auf $x$ und $x'$ aufgeteilt werden indem je die Wurzel der Anteile zu $x$ und $x'$ multipliziert werden.
Damit wurde gezeigt dass der \ac{RBF} Kernel das Skalarprodukt in einem unendlich dimensionalen Raum berechnet.
...
Diese Definition setzt voraus, dass die Funktion $\Phi$ bekannt ist.
Das Ziel der neuen Definition ist die Vermeidung von einzeln auftretenden transformierten Eingabevektoren $z$ in den Formeln um das gesamte Problem mittels einer Kernel-Funktion $K(x, x')$ ausdrücken zu können ohne die Funktion $\Phi$ tatsächlich kennen zu müssen.
...
Für $b$ ergibt sich durch Einsetzen von \autoref{eq:kernel_w} für einen beliebigen Stützvektor $x_{k}$:
...
\begin{tabular}{|l|}
    ...
    \hline
\end{tabular}\label{tab:pseudokernel}
\end{table}
...
$N$ ist hierbei die Anzahl der Eingabevektoren, $KF$ ist die Kernelfunktion.
...
\autoref{fig:rbfgammacomp} zeigt die Ergebnisse mit verschiedenen Werten für $\gamma$, angewandt auf den Datensatz KM40M2-NN.
\begin{figure}[H]
    \centering
    \includegraphics[width = 16cm]{../code/octave/images/sgdrbfkernelcomp}
    \caption{Nichtlinear separierbarer Datensatz mit Grenze von \ac{SVM} mit \ac{RBF}-Kernel für verschiedene $\gamma$}
    \label{fig:rbfgammacomp}
\end{figure}
Man sieht, dass sich bei manchen Werten für $\gamma$ mehrere kleinere abgegrenzte Bereiche bilden, aber nicht eine saubere Trenngrenze gezogen wird.
Mit $\gamma=1$ und $\gamma=1.5$ wird sauber getrennt.
Bei $\gamma=1$ werden die Punkte der Klasse 1 von einer Grenze umschlossen, bei $\gamma=1.5$ die Punkte der Klasse -1.\newline
% Weiter muss auch auf den Bestrafungsparameter $C$ geachtet werden.
Die Grenze für $\gamma = 1$ wird noch einmal separat in \autoref{fig:rbfkern_dataset} dargestellt. %TODO mention C=200?
\begin{figure}[H]
    \centering
...
Auch das Umschließen einer Klasse, das typisch für SVM mit RBF-Kernel ist, sieht man gut.\newline

\section{Kernel-Trick, Vergleich verschiedener Methoden}\label{sec:compkernelothers}
Vergleicht man die SVM mit Kernel Trick mit bereits aus der Vorlesung bekannten Methoden, erhält man \autoref{fig:compdatasetkernelothers}.
\begin{figure}[H]
    \centering
    \includegraphics[width = 16cm]{../code/octave/images/compbatchdecisionboundary}
    \caption{Nichtlinear separierbarer Datensatz mit Grenzen verschiedener Methoden}
    \label{fig:compdatasetkernelothers}
\end{figure}
Man sieht, dass alle Methoden trennen.
Wichtig ist hier, dass die SVM-Methoden stabiler sind, durch die Abwesenheit der zufällig initialisierten Gewichtsmatrix der anderen Methoden.
Vergleicht man die Zeit, die die Algorithmen brauchen, erhält man \autoref{fig:compbatchtimes}.
\begin{figure}[H]
    \centering
    \includegraphics{../code/octave/images/compbatchtimes}
    \caption{Dauer verschiedener Methoden für nichtlinearen Datensatz}
    \label{fig:compbatchtimes}
\end{figure}
Man sieht, dass die SVM-Methoden wesentlich schneller sind.
Das verwendete QP-Solver Framework trägt hier sicher einiges dazu bei.
Die Kombination aus kurzer Dauer und Stabilität macht das Experimentieren zum Finden der richtigen Hyperparameter ($\gamma$ oder $Q, a, b$) mit den SVM-Methoden angenehmer.
