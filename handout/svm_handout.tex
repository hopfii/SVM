...
Mit einer Hard-Margin SVM kann man beispielsweise linear separierbare Punkte trennen.
...
\end{figure}
Man sieht, dass die Grenze einen Mindestabstand zu allen Punkten (eben den "Hard margin") hat.
\autoref{fig:hmsvm_comp} zeigt einen Vergleich zwischen Hard-Margin SVM, einem Algorithmus mit Pseudo-inverse und $\alpha$-LMS.
\begin{figure}[H]
    \centering
    \includegraphics[width = 16cm]{../code/octave/images/linearcompsmall}
    \caption{Linear separierbarer Datensatz mit Grenze von Hard-Margin SVM, Pseudo-Inverser und $\alpha$-LMS}
    \label{fig:hmsvm_comp}
\end{figure}
Alle Entscheidungsgrenzen trennen den Datensatz sauber.
Im Gegensatz zur Hard-Margin SVM ist bei den anderen Algorithmen der Abstand der Punkte zur Grenze unerheblich.
...
Im Gegensatz zur Hard-Margin SVM kann bei der Soft-Margin SVM der Mindestabstand, oder genauer der Bestrafungsfaktor für Abweichungen von diesem, eingestellt werden.
So kann mit einer geringen Bestrafung auch bei nicht-linear separierbaren Datensätzen ein gutes Ergebnis erzielt werden, vorausgesetzt die Klassen sind nur leicht durchmischt.
Ein Beispiel, mit einer Grenze von einer Hard-Margin SVM zum Vergleich, sieht man in \autoref{fig:smsvm_notlinear}.
\begin{figure}[H]
    \centering
    \includegraphics[width = 16cm]{../code/octave/images/svmsimplenotlinear}
    \caption{Nicht linear separierbarer Datensatz mit Grenze von Hard-Margin SVM und Soft-Margin SVM}
    \label{fig:smsvm_notlinear}
\end{figure}
Man sieht, dass die Soft-Margin SVM trotz der zwei Punkte, die eine lineare Trennung verhindern, eine gute Grenze zieht.
Die beiden Ausreißer werden quasi ignoriert.
Anders bei der Hard-Margin SVM, die durch die Ausreißer stark beeinträchtigt wird.
Stellt man den Bestrafungsparameter $C$ sehr hoch ein, verhält sich eine Soft-Margin SVM genau wie eine Hard-Margin SVM.
...
Beim polynomiellen Kernel gilt es herauszufinden, durch welches Polynom sich eine Trennung der eigentlich nichtlinearen Problemstellung erreichen läßt.
...
\begin{figure}[H]
    ...
    \caption{Nicht linear separierbarer Datensatz mit Grenze von SVM mit polynomiellem Kernel für $Q=2,\ldots,6$}
    \label{fig:plykern_qvary}
\end{figure}
Man kann die verschiedenen Grenzen beobachten.
Da sich durch den Kerneltrick der Rechenaufwand in Grenzen hält, kann man über die Variation von $Q$ und gegebenfalls auch $a, b$ gut experimentieren, um ein geeignetes Polynom zu finden.
Bei genaurer Betrachtung bietet sich bei diesem Beispiel ein Exponent von $Q=4$ an.
In \autoref{fig:plykern_solve} sieht man nur diese Entscheidungsgrenze.
\begin{figure}[H]
    \centering
    \includegraphics[width = 16cm]{../code/octave/images/sgddatasetkernelsolve}
    \caption{Nicht linear separierbarer Datensatz mit Grenze von SVM mit polynomiellem Kernel für $Q=4$}
    \label{fig:plykern_solve}
\end{figure}
Man kann sehen, dass die Punkte sauber getrennt werden, auch bei einem nichtlinearen Problem.
...
Anders als beim polynomiellen Kernel muss beim RBF Kernel nicht nach einem Polynom gesucht werden.
...
\begin{figure}[H]
    ...
    \caption{Nicht linear separierbarer Datensatz mit Grenze von SVM mit RBF Kernel für $\gamma=1$}
