...
\subsection{Soft Margin \ac{SVM}} \label{sec:soft_margin}
Bisher wurde angenommen, dass alle Eingabevektoren linear separierbar sind. Dieses Problem kann durch eine Hard-Margin SVM, wie in den Kapiteln zuvor gezeigt gelöst werden. Trifft diese Annahme für eine Menge von Eingabevektoren nicht mehr zu wird der bisher beschriebene Algorithmus keine Trennebene finden. Durch die Einführung von positiven Fehlervariablen $\xi_{n} \in \mathbb{R}^{K}, \xi_{n} \geq 0$ in \autoref{decision_rules} kann dieses Problem umgangen werden:
...
Damit ein Eingabevektor $x_{n}$ nun, gemäß \autoref{decision_rules_softmargin}, falsch klassifiziert werden kann muss der Fehler $\xi_{n}$ über $1$ steigen. Eine obere Grenze für die Anzahl der aufgetretenen Fehler kann somit beschrieben werden durch $\sum_{n=1}^{N} \xi_{n}$. \\
