...
Durch \autoref{decision_rules} wird sichergestellt, dass Werte aus dem Intervall $\interval[open]{-1}{+1}$ als Ergebnis nicht vorkommen können. Dadurch entsteht eine Art symmetrisches Trennband um die Trennebene in dem keine Punkte liegen dürfen. \\ \autoref{decision_rules} kann weiter verallgemeinert werden durch beidseitige Multiplikation mit $y_{k}$:
...
\autoref{distance_to_plane_simplified2} beschreibt den Normalabstand zu dem am nächsten an der Ebene liegenden Punkt $\hat{x}$, der gleichbedeutend mit der Breite des Trennbands ist. Ziel einer \ac{SVM} ist die Maximierung dieses Abstands für $N$ Eingabevektoren $\{x_{1}..x_{N}\}, x_{n} \in \mathbb{R}^{D}$. Bei dieser Version der \ac{SVM} müssen die Eingabevektoren linear separierbar sein. Hierbei handelt es sich um ein Optimierungsproblem mit Nebenbedingungen:
